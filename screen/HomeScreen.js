...
import axios from 'axios'; // for API fetch
...
const HomeScreen = ({ navigation, route }) => {
  ...
  const handleLogout = () => {
    Alert.alert('Logout', 'Do you really want to logout?', [
      ...
      { text: 'Logout', onPress: () => navigation.replace('Userlogin') }
    ]);
  };
  ...
  useEffect(() => {
    ...
    if (email) {
      axios
        .get(`http://10.0.2.2:3000/api/auth/showuser`, { params: { email } })
        .then((res) => {
  ...
  const handleScanPress = () => {
    scanButtonRef
      ? scanButtonRef.pulse(200).then(() => navigation.navigate('ScanCard'))
      : navigation.navigate('ScanCard');
  };

  const handlecard = () => {
    enterCardRef
      ? enterCardRef.pulse(200).then(() => navigation.navigate('EnterCard'))
      : navigation.navigate('EnterCard');
  };

  const handleList = () => {
    listRef
      ? listRef.pulse(200).then(() => navigation.navigate('VisitorList'))
...
const styles = StyleSheet.create({
  ...
  emojiIcon: { fontSize: 32, marginRight: 5 },
  topWelcome: { fontSize: 28, fontWeight: '600', color: '#093FB4' },
  innerContainer: {
    flex: 1,
  ...
  heading: { fontSize: 28, fontWeight: 'bold', marginBottom: 20, color: '#093FB4' },
  animatedButton: { borderRadius: 10, overflow: 'hidden' },
  button: {
    padding: 14,
    borderRadius: 40,
    alignItems: 'center',
  ...
  buttonText: { color: '#fcfcfdff', fontSize: 18, fontWeight: '700' },
  logoutButton: {
    marginTop: 170,
    paddingVertical: 12,
